...
def remove_user(group_id, user_id):
    try:
        ...
        content, status = {'msg': GROUP['users']['remove']['failure']['not_found']}, 404
    except AssertionError as e:
        content, status = {'msg': GROUP['users']['remove']['failure']['assertions'].format(reason=e)}, 422
    except Exception as e:
        log.critical(e)
        content, status = {'msg': G['internal_error']}, 500
    else:
        content, status = {'msg': GROUP['users']['remove']['success'], 'group': group.as_dict}, 201
    finally:
