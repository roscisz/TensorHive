...
from tensorhive.models import Task
...
class Job(CRUDModel, Base):  # type: ignore
    ...
    def __repr__(self):
        return '<Task id={id}, name={name}, description={description}, user={user_id}, status={status}, start_at={start_at}, terminate_at={terminate_at}>'.format(
                id=self.id,
                name=self.name,
