...
def test_remove_task_from_job(tables, client, new_job_with_task):
    task = new_job_with_task.tasks[0]
    resp = client.delete(ENDPOINT + '/{}/tasks/{}'.format(new_job_with_task.id, task.id), headers=HEADERS)

    assert resp.status_code == HTTPStatus.OK
    assert task.job is None
    assert len(new_job_with_task.tasks) == 0
